...
class Deck {
    ...
    public Deck(int n)
    {
        ...
        makeOriginalDeck(numberDecks);  // this private method includes try { } catch() { }
    ...
    public List<Card> deal(int numberCards) throws PlayingCardException
    {
        ...
        assert numberCards > 0;

        List<Card> dealtCards = new ArrayList<Card>();
        int fromIndex = playingDeck.size() - numberCards;   // inclusive
        int toIndex = playingDeck.size();   // don't minus 1, exclusive
        dealtCards.addAll(playingDeck.subList(fromIndex, toIndex)); // call subList() and addAll() 
        playingDeck.removeAll(dealtCards);  // call removeAll()
    ...
    public void reset()
    {
        ...
        List<Card> tempList = (List<Card>) ((ArrayList<Card>) originalDeck).clone();
        playingDeck = tempList;
    }
    ...
    private void makeOriginalDeck(int n) {
        try {
            for (int i = 0; i < n; i++)    // n decks
                for (int j = 1; j <= 4; j++)   // suits
                    for (int k = 1; k <= 13; k++)  // ranks
                        originalDeck.add(new Card(k, j));
        } catch (PlayingCardException e) {
            System.out.println("PlayingCardException: " + e.getMessage());
        }
    }
